...
class ArrestinActiveCA(DataSet):
    ...
    top = loc+'arr2-active_start_centered_CA_bonds.pdb'
    trj = loc+'arr2-active_output_aligned_final_CA_bonds.xtc'
    log = loc+'arr2-active_log.txt'

    def __init__(self, root=os.getcwd()):
        super(ArrestinActiveCA, self).__init__(root=root)
        self._system = system_by_name("ArrestinActiveCA")
...
class ArrestinInactiveCA(DataSet):
    ...
    loc = '/oak/stanford/groups/rondror/projects/ensemble-generators/arrestin/openmm-arrestin/'
    top = loc+'arr2-inactive_start_centered_CA_bonds.pdb'
    trj = loc+'arr2-inactive_output_aligned_final_CA_bonds.xtc'
    log = loc+'arr2-inactive_log.txt'

    def __init__(self, root=os.getcwd()):
        super(ArrestinInactiveCA, self).__init__(root=root)
        self._system = system_by_name("ArrestinInactiveCA")
        self._temperature = 310
        self._xyz = mdtraj.load_xtc(self.trj, self.top).xyz
